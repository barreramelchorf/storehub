...
export default function AnalyticsPage() {
  ...
  const { data: categoryData } = useQuery({
    ...
    queryFn: () => {
      if (period === 'year') {
        ...
        return api(`/api/admin/analytics/categories?from=${from}&to=${to}`, { token })
      }
      return api(`/api/admin/analytics/categories?from=${range.from}&to=${range.to}`, { token })
    },
  ...
  return (
    <div>
      ...
      {/* Yearly view */}
      {period === 'year' && yearData && <YearlyView data={yearData} categoryData={categoryData} />}
...
function YearlyView({ data, categoryData }: { data: any; categoryData?: any }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Category analytics for the year */}
      {categoryData?.salesByCategory?.length > 0 && (
        <div className="card p-5">
          <h2 className="text-sm font-semibold text-[var(--color-text-dark)] mb-4">Ventas por categoría ({data.year})</h2>
          <div className="space-y-2">
            {[...categoryData.salesByCategory].sort((a: any, b: any) => Number(b.totalRevenue) - Number(a.totalRevenue)).map((cat: any, i: number, arr: any[]) => {
              const maxRevenue = Number(arr[0].totalRevenue)
              const barWidth = maxRevenue > 0 ? (Number(cat.totalRevenue) / maxRevenue) * 100 : 0

...
export async function platformRoutes(app: FastifyInstance) {
  ...
  app.post('/api/platform/tenants', async (request, reply) => {
    ...
    const [adminRole] = await db.insert(roles).values({
      ...
      permissions: ['sales.create','sales.view','sales.override_price','sales.backdate','inventory.view','inventory.manage','inventory.restock','analytics.view','audit.view','settings.manage','users.manage','documents.manage'],
    }).returning()

    await db.insert(roles).values({ tenantId: tenant.id, name: 'manager', isDefault: true, permissions: ['sales.create','sales.view','sales.override_price','sales.backdate','inventory.view','inventory.manage','inventory.restock','analytics.view','audit.view','documents.manage'] })

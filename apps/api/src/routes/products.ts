...
export async function productRoutes(app: FastifyInstance) {
  ...
  app.delete('/api/admin/products/:id', { preHandler: requirePermission('inventory.manage') }, async (request, reply) => {
    ...
    try {
      ...
      if (e.code === '23503') {
        return reply.code(400).send({ error: 'Este producto tiene ventas registradas y no se puede eliminar. Desactívalo desde Editar si no quieres que aparezca en el POS.' })
      }
      throw e
    }
  })

...
const FROM_EMAIL = process.env.RESEND_FROM_EMAIL ?? 'StoreHub <notificaciones@barreramelchorf.top>'
...
export async function sendPasswordChangedEmail(params: PasswordChangedEmailParams) {
  ...
  const { to, username, password, tenantName, loginUrl, changedBy, mustChangePassword } = params

  try {
    await resend.emails.send({
      from: FROM_EMAIL,
      to,
      subject: `Tu contraseña en ${tenantName} ha sido actualizada`,
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px;">
          <h1 style="font-size: 24px; font-weight: bold; color: #0A2540; margin-bottom: 8px;">Contraseña actualizada</h1>
          <p style="color: #425466; font-size: 16px; line-height: 1.5;">
            ${changedBy} ha actualizado tu contraseña en ${tenantName}. Aquí están tus nuevos datos de acceso:
          </p>
          ...
          <div style="background: #FFF8E1; border-radius: 8px; padding: 16px; margin: 24px 0; border-left: 4px solid #F59E0B;">
            <p style="margin: 0; color: #92400E; font-size: 14px;">
              ⚠️ Por seguridad, se te pedirá cambiar tu contraseña en tu próximo inicio de sesión.
            </p>
          </div>
          ` : ''}
